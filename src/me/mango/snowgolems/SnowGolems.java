...
public class SnowGolems extends JavaPlugin {
	private final SnowGolemPlayerListener playerListener = new SnowGolemPlayerListener(
			this);
			...
			if (commandName.equalsIgnoreCase("sg")) {
				if (!snowPlayers.contains((Player) sender)) {
					...
					sender.sendMessage(ChatColor.GREEN
							+ "You have enabled Snow Golem mode!");
					return true;
				} else {
					if (snowPlayers.contains((Player) sender)) {
						snowPlayers.remove((Player) sender);
						sender.sendMessage(ChatColor.DARK_RED
								+ "You have disabled Snow Golem mode.");
						return true;
					} else {
				...
				if (commandName.equalsIgnoreCase("sgdisable")) {
					...
					if (commandName.equalsIgnoreCase("sghelp")) {
						...
						sender.sendMessage(ChatColor.GREEN
								+ "/sg - Toggles Snow Golem mode. (Requires OP)");
						sender.sendMessage(ChatColor.GREEN
								+ "/sghelp - Displays this help message.");
						return true;
					}
				}
			} else {
				sender.sendMessage(ChatColor.RED
						+ "Only OPs are allowed to use that command!");
				return true;
			}
			return false;
		}
	...
	public class SnowGolemPlayerListener extends PlayerListener {
		...
		public void onPlayerMove(PlayerMoveEvent event) {
			...
			if (snowPlayers.contains((Player) player)) {
				...
				b.setTypeId(78); // snow (the kind that falls from the sky, not
									// the block)
			}
		}
	}
